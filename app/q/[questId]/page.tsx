...
export default function Home({ params }: {params: {questId:string}}) {
    ...
    useEffect(()=>{
        if(!user) return
        user.getIdToken()
        .then((s) => fetch(`/api/quest/${qid}/host`,{
                headers: { 
            ...
            <>
                <Card className="w-full m-4">
                    <CardHeader style={{
                        ...
                        {(qData.active)?<div className={`${font.className} text-black bg-white rounded-md pl-2 pr-2 flex items-center gap-2`}> <span className="text-green-600 text-2xl">●</span> Live</div>:
                        <div className={`${font.className} text-black bg-white rounded-md pl-2 pr-2 flex items-center gap-2`}> <span className="text-red-600 text-2xl">●</span> Unavailable</div>}
